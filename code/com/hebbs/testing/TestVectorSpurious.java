...
public class TestVectorSpurious {
	...
	public double[][] Inputdataflip(String fname){
	...
	public void displayInput(Matrix m, int row){
	...
	public void displayInput(Matrix ip, Matrix op, int row, PrintWriter out){
		...
		for(int i=0;i<35;i++){
			...
			if(op.get(row, i) == 1)
				out.print("#");
			else if(op.get(row, i) == -1)
				out.print(" ");
			else if(op.get(row, i) == 0)
				out.print("0");
		}
	}
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		TestVectorSpurious tv = new TestVectorSpurious();
		int[] row = {0,2,3,5,8};
		...
		for(int run = 0; run < 10000;run++){
			...
			Matrix errorInput = tv.randomError(input, 4, out1);
			
			//Testing the Error Input
			double[][] dataflip = tv.Inputdataflip("input.txt");
			Matrix flipInput = new Matrix(row.length,35);
			for(int i=0;i<row.length;i++)
				for(int j=0;j<35;j++)
					flipInput.set(i, j, dataflip[row[i]][j]);
				
			Matrix errorOutput = errorInput.times(weight);
			errorOutput = tv.addBias(errorOutput, bias);
			//Matrix errorOutput = tv.randomData().times(weight);
			boolean flag = false;
			int count=0;
			//for(int i=0;i<row.length;i++){
			...
			for(int i=0;i<row.length;i++){
				...
				flag = true;
				//tv.displayInput(flipInput, i);
				out.print("\nErr Ip Output");
				tv.displayInput(errorInput, errorOutput, i, out1);
				for(int j=0;j<35;j++){
					
					if(errorInput.get(i, j) != errorOutput.get(i, j)){
